// input: 路由状态、批量结果集合、结果视图映射与结果页组件。
// output: 固定视口内完整展示的单人/批量排盘结果页面。
// pos: 前端结果展示页面容器。
...
import type { BatchChartResultItem, BirthChartApiResponse, BirthFormValue, ChartMode, ChartRecordDetailResponse } from "../types/models";
...
interface ResultRouteState {
  ...
  batchItems?: BatchChartResultItem[];
}
...
export function ResultPage() {
  ...
  const batchItems = routeState.batchItems ?? [];
  const activeBatchItem = batchItems[activeBatchIndex] ?? null;
  const payload = activeBatchItem?.payload ?? routeState.payload;
  const formValue = activeBatchItem?.formValue ?? routeState.formValue;
  const recordId = activeBatchItem?.recordId ?? routeState.recordId;
  const recordAction = activeBatchItem?.recordAction ?? routeState.recordAction;
  ...
  const hasBatchItems = batchItems.length > 1;
  ...
  return (
    <main className="h-dvh overflow-hidden px-3 py-3 sm:px-5 lg:px-6">
      <div className="mx-auto flex h-full max-w-[min(96vw,92rem)] flex-col">
        <div className="mb-2 flex shrink-0 items-center justify-between">
          <div>
            <p className="font-display text-xs font-bold uppercase tracking-[0.24em] text-plum/60">Nine Grid System</p>
            <h1 className="mt-1 font-display text-xl font-extrabold tracking-tight text-ink">九宫格排盘结果</h1>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2">
            <button
              type="button"
              onClick={handleReturnToInput}
              className="rounded-full border border-plum/15 bg-white/80 px-3.5 py-1.5 text-xs font-semibold text-plum transition hover:bg-plum/5"
            >
              返回首页
            </button>
            <UserAccountPanel />
        ...
        <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1.08fr)_minmax(23rem,0.92fr)]">
          <section className="card-surface flex h-full min-h-0 flex-col p-3">
            <div className="shrink-0 rounded-[18px] border border-white/75 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(247,241,252,0.92))] px-3 py-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {hasBatchItems ? (
                  <div className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-plum/10 bg-white/82 p-0.5 shadow-sm">
                    <span className="px-2 text-xs font-semibold uppercase tracking-[0.16em] text-plum/55">批量结果</span>
                    {batchItems.map((item, index) => {
                      const active = index === activeBatchIndex;
                      return (
                        <button
                          key={`${item.formValue.name}-${item.formValue.birthDate}-${index}`}
                          type="button"
                          onClick={() => setActiveBatchIndex(index)}
                          className={`rounded-full px-2.5 py-1 text-xs font-display font-bold transition ${
                            active ? "bg-plum text-white shadow-sm" : "text-ink/58 hover:bg-plum/5 hover:text-plum"
                          }`}
                        >
                          {item.formValue.name || `第 ${index + 1} 位`}
                        </button>
                      );
                    })}
                  </div>
              ...
              <div className="mt-2 space-y-1">
                {recordAction === "created" && recordId ? (
                  <div className="rounded-[14px] border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-800">
                    当前结果已保存为档案 #{recordId}。
                  </div>
                ) : null}
                ...
                {!isAuthenticated ? (
                  <div className="rounded-[14px] border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-medium text-amber-800">
      ...
      <button
        ...
        className="fixed bottom-3 left-1/2 z-40 inline-flex -translate-x-1/2 items-center justify-center rounded-full bg-gradient-to-r from-plum to-iris px-5 py-2.5 font-display text-sm font-bold text-white shadow-soft transition hover:translate-x-[-50%] hover:translate-y-[-1px]"
      >
        九宫格AI问答系统
      </button>
      {formValue ? (
        <button
          ...
          className="fixed bottom-3 right-4 z-40 rounded-full bg-gradient-to-r from-plum to-iris px-4 py-2.5 font-display text-xs font-bold text-white shadow-soft transition hover:translate-y-[-1px] disabled:cursor-not-allowed disabled:opacity-60"
        >
          新增档案
        </button>
      ) : null}
